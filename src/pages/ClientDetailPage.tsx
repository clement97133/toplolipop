...
function EventFormModal({ clientId, open, onClose }: { clientId: string; open: boolean; onClose: () => void }) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['events', 'client', clientId] }); onClose() },
  })
...
export default function ClientDetailPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <button onClick={() => navigate('/clients')} className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 transition-colors mb-5">
        ...
        <div className="lg:col-span-1 space-y-4">
          <Card>
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Depuis le {formatDate(client.created_at)}</p>
                </div>
              </div>
            ...
            <div className="space-y-2.5">
              {client.phone && (
                <a href={`tel:${client.phone}`} className="flex items-center gap-2.5 text-sm text-gray-600 hover:text-brand-600 transition-colors group">
                  <Phone size={14} className="text-gray-400 group-hover:text-brand-500" />{client.phone}
                </a>
              )}
              {client.email && (
                <a href={`mailto:${client.email}`} className="flex items-center gap-2.5 text-sm text-gray-600 hover:text-brand-600 transition-colors group truncate">
                  <Mail size={14} className="text-gray-400 group-hover:text-brand-500 flex-shrink-0" />{client.email}
                </a>
              )}
              {client.address && (
                <div className="flex items-start gap-2.5 text-sm text-gray-600">
                  <MapPin size={14} className="text-gray-400 mt-0.5 flex-shrink-0" />{client.address}
                </div>
              )}
            </div>
            ...
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-4 pt-4 border-t border-gray-50">
                {tags.map((tag) => (
                  <Badge key={tag} className="bg-brand-50 text-brand-600">{tag}</Badge>
                ))}
              </div>
            )}
          </Card>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Card padding="sm" className="text-center">
              <p className="text-xl font-bold text-brand-600">{events.length}</p>
              <p className="text-xs text-gray-500 mt-0.5">Événements</p>
            </Card>
            <Card padding="sm" className="text-center">
              <p className="text-xl font-bold text-blue-600">{upcoming}</p>
          ...
          {activeTab === 'info' && (
            <div className="space-y-4">
              {client.event_address && (
                <Card>
                  <CardHeader title="Adresse des événements" />
                  <p className="text-sm text-gray-600">{client.event_address}</p>
                </Card>
              )}
              {client.preferences && (
                <Card>
                  <CardHeader title="Préférences" />
                  <p className="text-sm text-gray-600 whitespace-pre-wrap">{client.preferences}</p>
                </Card>
              )}
          ...
          {activeTab === 'events' && (
            <div>
              <div className="flex justify-end mb-4">
                <Button size="sm" onClick={() => setShowEventForm(true)}><Plus size={14} />Ajouter un événement</Button>
              </div>
              {events.length === 0 ? (
                <EmptyState icon={Calendar} title="Aucun événement" description="Ajoutez le premier événement de ce client." action={<Button size="sm" onClick={() => setShowEventForm(true)}><Plus size={14} />Ajouter</Button>} />
              ) : (
                <div className="space-y-2">
                  {events.map((ev) => (
                    <div key={ev.id} className="bg-white rounded-xl border border-gray-100 shadow-card p-4 hover:shadow-card-hover transition-all">
          ...
          {activeTab === 'documents' && (
            <div>
              {docs.length === 0 ? (
                ...
                <div className="space-y-2">
                  {docs.map((doc) => (
                    <div key={doc.id} className="bg-white rounded-xl border border-gray-100 shadow-card p-4 hover:shadow-card-hover transition-all">
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="font-mono text-xs font-medium text-gray-500">{doc.number}</p>
                            <Badge className="bg-gray-100 text-gray-600">{DOCUMENT_TYPE_LABELS[doc.type]}</Badge>
                            <Badge className={DOCUMENT_STATUS_COLORS[doc.status]}>{DOCUMENT_STATUS_LABELS[doc.status]}</Badge>
                          </div>
                          {doc.title && <p className="text-sm font-medium text-gray-800 mt-1">{doc.title}</p>}
                        </div>
                        <p className="text-lg font-bold text-gray-900">{formatCurrency(doc.total)}</p>
                      </div>
                    </div>
                  ))}
                </div>
